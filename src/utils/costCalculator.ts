...
const depreciationRates: DepreciationRates = {
  Electronics: 0.4,
  Clothes: 0.0769,
  Kitchenware: 0.1277,
  Furniture: 0.1277,
  Books: 0.4,
  Stationary: 0.0769, // according to the categories later listed
};

const calculateCost = (product: ProductDocument): number => {
  // very basic category classification
  if (
    // product.tags.test(/phone/i) ||
    // product.tags.test(/tv/i) ||
  ...
  const multiplier = depreciationRates[product.category]
    ? depreciationRates[product.category]
    : 0.23904;
